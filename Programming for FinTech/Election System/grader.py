...
class TestElectionEvalPregrader(unittest.TestCase):
    ...
    def test_tie_vote(self):
        ...
        state_info_file = "tie_state_info.txt"
        vote_info_file  = "tie_vote_info.txt"
        solution_python_file =  "solution.py"
        ...
        student_result = subprocess.run(command.split(),stdin=subprocess.PIPE, stdout=subprocess.PIPE, encoding='utf8')
        solution_result = subprocess.run(solution_command.split(), stdin=subprocess.PIPE, stdout=subprocess.PIPE, encoding='utf8')
        amended = student_result.stdout.replace("Collage","College")
        self.assertEquals(amended, solution_result.stdout, "Test electoral college tie: output does not match the expected output.")
        global grade
        grade += 15

    def test_no_states_won_by_a(self):
        """Test the output when a candidate a doesn't win any states """
        state_info_file = "nowin_state_info.txt"
        vote_info_file  = "nowin_vote_info.txt"
        solution_python_file =  "solution.py"
    ...
    def test_invalid_command_line_arguments(self):
        """Test that the number of command line arguments are validated """
        global grade
        command = "python3 -u election.py test"
        student_result = subprocess.run(command.split(),stdin=subprocess.PIPE, stdout=subprocess.PIPE, encoding='utf8')
        self.assertEqual(student_result.returncode,1,"Invalid return code for missing arguments")
        grade += 3

        command = "python3 -u election.py test test test"
        student_result = subprocess.run(command.split(),stdin=subprocess.PIPE, stdout=subprocess.PIPE, encoding='utf8')
        grade += 4

        command = "python3 -u election.py "
        student_result = subprocess.run(command.split(),stdin=subprocess.PIPE, stdout=subprocess.PIPE, encoding='utf8')
        self.assertEqual(student_result.returncode,1,"Invalid return code for missing arguments")
        grade += 3
